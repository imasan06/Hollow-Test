...
export function createAudioBlobUrl(samples: Int16Array): string {
  const wav = encodeWav(samples);
  const blob = new Blob([new Uint8Array(wav).buffer as ArrayBuffer], {
    type: "audio/wav",
  });
...
export function validateWavFormat(wavBase64: string): {
  valid: boolean;
  sampleRate?: number;
  channels?: number;
  error?: string;
  ...
  try {
    ...
    if (sampleRate !== SAMPLE_RATE) {
      return {
        ...
        sampleRate,
        channels,
        error: `Sample rate must be ${SAMPLE_RATE}Hz, got ${sampleRate}Hz`,
      };
    }
    ...
    if (channels !== NUM_CHANNELS) {
      return {
        ...
        sampleRate,
        channels,
        error: `Must be mono (1 channel), got ${channels} channels`,
      };
    }
    ...
    if (bitsPerSample !== BITS_PER_SAMPLE) {
      return {
        ...
        sampleRate,
        channels,
        error: `Must be 16-bit, got ${bitsPerSample}-bit`,
      };
    }
